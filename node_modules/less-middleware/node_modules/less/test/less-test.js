...
runTestSet(null, "errors/", function(name, err, compiledLess, doReplacements) {
    fs.readFile(path.join('test/css/', name) + '.txt', 'utf8', function (e, expectedErr) {
        sys.print("- " + name + ": ");
        expectedErr = doReplacements(expectedErr, 'test/css/errors/');
        if (!err) {
            if (compiledLess) {
                fail("No Error", 'red');
            } else {
        ...
        return input.replace(
            "{path}", path.join(process.cwd(), "/test/css/errors/"))
            .replace("{pathrel}", path.join("test", "css", "errors/"))
            .replace(/\r\n/g, '\n');
    });
...
function runTestSet(options, foldername, verifyFunction, nameModifier, doReplacements) {
    ...
    fs.readdirSync(path.join('test/css/', foldername)).forEach(function (file) {
        if (! /\.less/.test(file)) { return }
        
        var name = foldername + path.basename(file, '.css');
        
        if (oneTestOnly && name !== oneTestOnly) { return; }
        
        totalTests++;

        toCSS(options, path.join('test/css/', foldername + file), function (err, less) {
            ...
            fs.readFile(path.join('test/css', css_name) + '.css', 'utf8', function (e, css) {
                ...
                css = css && doReplacements(css, 'test/css/' + foldername);
                if (less === css) { ok('OK'); }
                else if (err) {
