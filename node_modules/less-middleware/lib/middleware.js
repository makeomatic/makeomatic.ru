 ...
 *    `font_src`             Source directory used to find .css files
 *    `dest`            Destination directory used to output .css files
 *                      when undefined defaults to `font_src`.
 *    `prefix`          Path which should be stripped from `pathname`.
 *    `compress`        Whether the output .css files should be compressed
 *    `yuicompress`     Same as `compress`, but uses YUI Compressor
 *    `optimization`    The desired value of the css optimization option (0, 1, or 2. 0 is default)
 *    `dumpLineNumbers` Add line tracking to the compiled css. ('comments' or 'mediaquery')
 *
 * Examples:
 *
 * Pass the middleware to Connect, grabbing .css files from this directory
...
module.exports = less.middleware = function(options){
  ...
  // Source dir required
  var src = options.src;
  if (!src) { throw new Error('css.middleware() requires "font_src" directory'); }
  ...
  return function(req, res, next) {
    ...
    if (regex.handle.test(pathname)) {
      ...
      var lessPath = path.join(src, (
        ...
        ? pathname.replace(regex.compress, '.css')
        : pathname.replace('.css', '.css'
      )));
      ...
      fs.stat(lessPath, function(err, lessStats){
        ...
        fs.stat(cssPath, function(err, cssStats){
          ...
          if (err) {
            ...
            // Check if any of the css imports were changed
