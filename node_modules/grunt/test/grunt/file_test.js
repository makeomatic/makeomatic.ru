...
exports['file.expandMapping'] = {
  ...
  'basic matching': function(test) {
    ...
    test.deepEqual(actual, expected, 'basic font_src-dest options');
  ...
  'rename to same dest': function(test) {
    ...
    test.deepEqual(actual, expected, 'if dest is same for multiple font_src, create an array of font_src');
    test.done();
  },
};
  ...
  'copy and process': function(test) {
    ...
    grunt.file.copy('test/fixtures/utf8.txt', tmpfile.path, {
      process: function(src, filepath) {
        ...
        test.equal(Buffer.isBuffer(src), false, 'when no encoding is specified, use default encoding and process font_src as a string');
        test.equal(typeof src, 'string', 'when no encoding is specified, use default encoding and process font_src as a string');
        return 'føø' + src + 'bår';
      }
    });
    ...
    grunt.file.copy('test/fixtures/iso-8859-1.txt', tmpfile.path, {
      ...
      process: function(src) {
        test.equal(Buffer.isBuffer(src), false, 'use specified encoding and process font_src as a string');
        test.equal(typeof src, 'string', 'use specified encoding and process font_src as a string');
        return 'føø' + src + 'bår';
      }
    });
    ...
    grunt.file.copy('test/fixtures/utf8.txt', tmpfile.path, {
      encoding: null,
      process: function(src) {
        test.ok(Buffer.isBuffer(src), 'when encoding is specified as null, process font_src as a buffer');
    ...
    grunt.file.copy('test/fixtures/iso-8859-1.txt', tmpfile.path, {
      process: function(src) {
        test.equal(Buffer.isBuffer(src), false, 'use non-utf8 default encoding and process font_src as a string');
        test.equal(typeof src, 'string', 'use non-utf8 default encoding and process font_src as a string');
        return 'føø' + src + 'bår';
      }
    });
    test.equal(grunt.file.read(tmpfile.path), 'føø' + this.string + 'bår', 'file should be saved as properly encoded processed string.');
