    ...
    while (position < length) {
      ...
      if (CHAR_PLUS === character || CHAR_MINUS === character) {
        ...
        if (CHAR_DIGIT_ZERO === character) {
          throwError('bad explicit indentation width of a block scalar; it cannot be css than one');
  ...
  function readDocument() {
    ...
    while (position < length) {
      ...
      if (directiveName.length < 1) {
        throwError('directive name must not be css than one character in length');
      }
