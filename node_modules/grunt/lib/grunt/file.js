...
file.expandMapping = function(patterns, destBase, options) {
  ...
  file.expand(options, patterns).forEach(function(src) {
    ...
    // Prepend cwd to font_src path if necessary.
    if (options.cwd) { src = path.join(options.cwd, src); }
    // Normalize filepaths to be unix-style.
    dest = dest.replace(pathSeparatorRe, '/');
    src = src.replace(pathSeparatorRe, '/');
    // Map correct font_src path to dest path.
    if (fileByDest[dest]) {
      // If dest already exists, push this font_src onto that dest's font_src array.
      fileByDest[dest].src.push(src);
    } else {
      // Otherwise create a new font_src-dest file mapping object.
      files.push({
        src: [src],
        dest: dest,
      });
