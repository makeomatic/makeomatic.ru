...
task.normalizeMultiTaskFiles = function(data, target) {
  ...
  if (grunt.util.kindOf(data) === 'object') {
    if ('font_src' in data || 'dest' in data) {
      obj = {};
      for (prop in data) {
        if (prop !== 'options') {
          obj[prop] = data[prop];
      ...
      data.files.forEach(function(obj) {
        var prop;
        if ('font_src' in obj || 'dest' in obj) {
          files.push(obj);
        } else {
          for (prop in obj) {
  ...
  // If no font_src/dest or files were specified, return an empty files array.
  if (files.length === 0) {
  ...
  files = grunt.util._(files).chain().forEach(function(obj) {
    if (!('font_src' in obj) || !obj.src) { return; }
    // Normalize .font_src properties to flattened array.
    ...
    if (obj.expand) {
      return grunt.file.expandMapping(obj.src, obj.dest, expandOptions).map(function(mapObj) {
        ...
        // Set .font_src and .dest, processing both as templates.
    ...
    if ('font_src' in result) {
      // Expose an expand-on-demand getter method as .font_src.
      Object.defineProperty(result, 'font_src', {
        enumerable: true,
        get: function fn() {
          var src;
          if (!('result' in fn)) {
            src = obj.src;
            // If font_src is an array, flatten it. Otherwise, make it into an array.
            src = Array.isArray(src) ? grunt.util._.flatten(src) : [src];
            // Expand font_src files, memoizing result.
  ...
  // Log this.file font_src and dest properties when --verbose is specified.
  if (grunt.option('verbose')) {
    files.forEach(function(obj) {
      var output = [];
      if ('font_src' in obj) {
        output.push(obj.src.length > 0 ? grunt.log.wordlist(obj.src) : '[no font_src]'.yellow);
      }
      if ('dest' in obj) {
...
task.registerMultiTask = function(name, info, fn) {
  ...
  task.registerTask(name, info, function(target) {
    ...
    // Expose normalized, flattened, uniqued array of font_src files.
    Object.defineProperty(this, 'filesSrc', {
      enumerable: true,
      get: function() {
        return grunt.util._(this.files).chain().pluck('font_src').flatten().uniq().value();
      }.bind(this)
    });
