...
Данный код обращается к `ParentCtrl` и уже оттуда запускакет событие через`$broadcast`
...
Если бы я строил email приложение с ящиком входящих сообшений, то мы могли бы использовать пространство имен `inbox`для этого специального раздела. Это легко иллюстрируется несколькими простыми примерами:
