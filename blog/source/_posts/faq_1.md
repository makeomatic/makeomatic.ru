...
## Как писать интеграционные тесты с использование `$httpBackend` из набора `Angular-mocks`
Как обычно достаточно пространное описание в документации не дает четкого представления о том, как же все-таки писать делать
адекватные заглушки для ответа. Приводим ответ в виде примера:
```javascript
